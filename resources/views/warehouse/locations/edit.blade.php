...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">Edit Lokasi Gudang</h4>
        ...
        <form action="{{ route('warehouse-locations.update', ['warehouse_location' => $warehouse->id]) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('internal_id') is-invalid @enderror" 
                       id="internal_id" name="internal_id" value="{{ old('internal_id', $warehouse->internal_id) }}" required>
                @error('internal_id')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="tracking_type" 
                               ...
                               {{ old('tracking_type', $warehouse->tracking_type) == 'bin_separated' ? 'checked' : '' }} required>
                        <label class="form-check-label" for="tracking_bin_separated">
                            • Di dalam bin terpisah
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="tracking_type" 
                               id="tracking_warehouse_only" value="warehouse_only" 
                               {{ old('tracking_type', $warehouse->tracking_type) == 'warehouse_only' ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          id="description" name="description" rows="3">{{ old('description', $warehouse->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
