    ...
    public function up(): void
    {
        if (!Schema::hasTable('seed_histories')) {
            Schema::create('seed_histories', function (Blueprint $table) {
                $table->id();
                $table->foreignId('inventory_type_seed_id')->constrained('inventory_type_seeds')->onDelete('cascade');
                ...
                $table->foreignId('user_id')->constrained('users')->onDelete('cascade')->comment('User yang melakukan aksi');
                $table->timestamps();
