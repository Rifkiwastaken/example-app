...
/**
 * Find and fix all ->references('id') in migrations
 */

$migrationsPath = __DIR__ . '/database/migrations';

// Table to custom PK mapping
$tablePkMap = [
    'users' => 'user_id',
...
$stats = ['processed' => 0, 'modified' => 0, 'found' => []];
...
foreach ($files as $file) {
    ...
    // Find all ->references('id')->on('table_name')
    if (preg_match_all("/->references\('id'\)->on\('(\w+)'\)/", $content, $matches, PREG_SET_ORDER)) {
        foreach ($matches as $match) {
            $tableName = $match[1];
            if (isset($tablePkMap[$tableName])) {
                $customPk = $tablePkMap[$tableName];
                $oldPattern = "->references('id')->on('$tableName')";
                $newPattern = "->references('$customPk')->on('$tableName')";
                $content = str_replace($oldPattern, $newPattern, $content);
                $stats['found'][] = "$filename: $tableName.id -> $tableName.$customPk";
            }
        }
    }
...
echo "\n========================================\n";
echo "References('id') Fix Summary:\n";
echo "Processed: {$stats['processed']}\n";
echo "Modified: {$stats['modified']}\n";
if (!empty($stats['found'])) {
    echo "\nFixed references:\n";
    foreach ($stats['found'] as $fix) {
        echo "  - $fix\n";
    }
}
echo "========================================\n";
